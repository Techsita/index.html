...
function portEval1 (e) { // NB ignoring all security...
  ...
  try { 
    ...
    if ( replyto && pmflag ) { // win, so po no reply at all (no recurse risk)...
      if (e.ports.length) { // quickie version of if got tranferable port, then mc initial eval replyto
        ...
        if (replyto === self) { // pmflag laziness, ie implies can assert we are Window, not s/worker or channel
